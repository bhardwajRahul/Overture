...
4. Call get_approval
   ...
   - If status is "approved", proceed with execution
   - If status is "cancelled", stop and inform user
5. Parse returned data:
   - fieldValues: User inputs for dynamic fields
   - selectedBranches: Which branch was selected at each decision node
   - nodeConfigs: Per-node attachments and meta instructions
6. For each node in execution order:
   a. Skip if node's branch wasn't selected
   b. Call update_node_status(node_id, "active")
   c. Execute the work using fieldValues and nodeConfigs
   d. Call update_node_status(node_id, "completed", output_summary)
